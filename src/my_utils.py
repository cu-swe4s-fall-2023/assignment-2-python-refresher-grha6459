...
def get_column(file_name, query_value, result_column, query_column=1):
    ...
    :return: a list of the numbers found in the result_column, if non-numerical values are found, will include NaN
             if issues are with file handeling are encountered, function will  return empty array
    """
    ...
    return np.array(result_list)  # Has been updated again to return numpy array


def get_mean(int_array):
    """
    :param int_array: A 1D numpy array of integers
    :return: The arithmetic mean of the array of integers, calculated using numpy
    Notes:
...
def get_median(int_array):
    """
    :param int_array: A 1D numpy array of integers
    :return: The median of the array of integers, as calculated using numpy library
...
def get_std_dev(int_array):
    """
    :param int_array: A 1D numpy array of integers
    :return: The standard deviation of the array of integers, as calculated using numpy a
    """
    if np.shape(int_array)[0] == 0:
        return None
    elif np.any(np.isnan(int_array)):
